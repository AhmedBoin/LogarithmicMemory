...
class LogarithmicMemory(BaseLogarithmicMemory):
    ...
    def parallel(self, x: Tensor) -> Tensor: # [batch, seq, embedding]
        ...
        for i in range(1, seq_x.bit_length()): # 1+floor(log(seq_x))
            ...
            x_temp = x_last.unsqueeze(-2).repeat(1, 1, 1, 2**i, 1).view(batch, bank, -1, embedding) # [batch, bank, seq_x, embedding]
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        model = LogarithmicMemory(embed_dim, heads, memory_banks)#.to("mps")
